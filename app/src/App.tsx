...
function App() {
  ...
  const addActivity = (kind: ActivityKind, description: string, taskId?: number) => setActivities((current) => [...current, { id: `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, kind, description, taskId, occurredAt: new Date().toISOString() }].slice(-MAX_LOCAL_ACTIVITIES));
  ...
  useEffect(() => { let cancelled = false; void loadGoogleSheets().then((remote) => { if (cancelled || !remote) return; const remoteTasks = Array.isArray(remote.tasks) ? remote.tasks : []; const remoteActivities = Array.isArray(remote.activities) ? remote.activities.slice(-MAX_LOCAL_ACTIVITIES) : activitiesRef.current; const remoteFocus = remote.focusByDate || {}; setTasks(remoteTasks); if (Array.isArray(remote.activities)) setActivities(remoteActivities); setFocusByDate(remoteFocus); setDailyInsights(hydrateInsights(remoteTasks, remoteActivities, Array.isArray(remote.insights) ? remote.insights : [], remoteFocus)); setFocusSeconds(Number(remoteFocus[jakartaDate()]) || 0); if (typeof remote.settings?.darkMode === "boolean") setDarkMode(remote.settings.darkMode); }).catch(() => { /* Local data remains available when offline or the deployment is not readable. */ }).finally(() => { if (!cancelled) setRemoteReady(true); }); return () => { cancelled = true; }; }, []);
