...
function syncGoogleSheets(tasks: Task[], insights: DailyInsight[], activities: Activity[]) {
  if (!SHEETS_URL) return;
  const payload = { tasks, insights, activities };
...
function App() {
  ...
  useEffect(() => { addActivity("page_viewed", `Membuka halaman ${page}`); }, [page]);
  ...
  useEffect(() => { localStorage.setItem(FOCUS_STORAGE_KEY, JSON.stringify(focusByDate)); }, [focusByDate]);
  ...
  useEffect(() => { addActivity("login", "Aplikasi dibuka"); }, []);
  useEffect(() => { if (!SHEETS_URL) return; const timer = window.setTimeout(() => syncGoogleSheets(tasksRef.current, insightsRef.current, activitiesRef.current), 400); return () => window.clearTimeout(timer); }, [tasks, activities]);
  ...
  useEffect(() => { const timers = tasks.filter((task) => !task.done && task.reminderAt).map((task) => { const delay = new Date(`${task.date}T${task.reminderAt}:00`).getTime() - Date.now(); return delay > 0 && delay < 2_147_000_000 ? window.setTimeout(() => { if (notificationReady) new Notification("FocusFlow · Task reminder", { body: `Saatnya mengerjakan: ${task.title}`, icon: "/vite.svg", tag: `focusflow-task-${task.id}`, requireInteraction: true }); setReminderToast(task); }, delay) : undefined; }).filter((timer): timer is number => timer !== undefined); return () => timers.forEach(window.clearTimeout); }, [notificationReady, tasks]);
  useEffect(() => {
    if (!SHEETS_URL) return;
    const syncForToday = () => { syncGoogleSheets(tasksRef.current, insightsRef.current, activitiesRef.current); localStorage.setItem(SHEETS_LAST_SYNC_KEY, jakartaDate()); };
  ...
  return <div className="app-shell">
    {reminderToast && <aside className="reminder-toast" role="alert"><strong>{reminderToast.title}</strong><button onClick={() => setReminderToast(null)}>×</button></aside>}
    ...
    <main className="content">{page === "calendar" ? <CalendarView tasks={tasks} openNewTask={openNewTask}/> : page === "insights" ? <InsightsView tasks={tasks} focusSeconds={focusSeconds} insights={dailyInsights}/> : page === "settings" ? <SettingsView darkMode={darkMode} setDarkMode={setDarkMode} addActivity={addActivity}/> : <>
      ...
      <aside className="right-column"><section className="timer-card card"><div className="timer-heading"><div><h2>Focus timer</h2><p>{activePeriod ? `${activePeriod} is running` : "Stay in the zone"}</p></div></div><div className="timer-tabs">{(Object.keys(DURATIONS) as Period[]).map((item) => <button onClick={() => setPeriod(item)} className={period === item ? "active" : ""} key={item}>{item}</button>)}</div><div className="timer-wrap"><div className="timer-ring"><div><strong>{time}</strong><span>{period}</span></div></div></div><div className="timer-controls"><button className="reset" onClick={() => { setTimerValues((values) => ({ ...values, [period]: DURATIONS[period] })); if (activePeriod === period) setActivePeriod(null); addActivity("timer_reset", `Timer ${period} direset`); }}><Icon name="rotate"/></button><button className="start" onClick={() => { const stopping = activePeriod === period; setActivePeriod(stopping ? null : period); addActivity(stopping ? "focus_paused" : "focus_started", `${period} ${stopping ? "dijeda" : "dimulai"}`); }}><Icon name={activePeriod === period ? "pause" : "play"}/>{activePeriod === period ? "Pause" : "Start"}</button></div></section><section className="quote-card"><span>“</span><div className="quote-content"><p>{QUOTES[quoteIndex].text}</p><small>— {QUOTES[quoteIndex].author}</small></div><div className="quote-controls"><button onClick={() => setQuoteIndex((quoteIndex - 1 + QUOTES.length) % QUOTES.length)}>←</button><button onClick={() => setQuoteIndex((quoteIndex + 1) % QUOTES.length)}>→</button></div></section></aside></div>
    </>}</main>
