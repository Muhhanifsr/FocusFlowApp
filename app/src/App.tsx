...
function App() {
  ...
  function toggleTask(id: string) { const task = tasks.find((item) => item.id === id); if (!task) return; setTasks((current) => current.map((item) => item.id === id ? { ...item, done: !item.done, completedAt: !item.done ? new Date().toISOString() : undefined } : item)); addActivity(task.done ? "task_reopened" : "task_completed", `${task.done ? "Task dibuka kembali" : "Task selesai"}: ${task.title}`, id); }
