...
type Task = { id: string; title: string; category: string; priority: Priority; done: boolean; date: string; reminderAt?: string; createdAt: string; completedAt?: string };
...
type Activity = { id: string; kind: ActivityKind; occurredAt: string; description: string; taskId?: string };
...
const MAX_LOCAL_ACTIVITIES = 500;
const MAX_LOCAL_INSIGHTS = 400;
...
const createTaskId = () => typeof crypto?.randomUUID === "function" ? crypto.randomUUID() : `${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
...
async function syncGoogleSheets(tasks: Task[], insights: DailyInsight[], focusByDate: Record<string, number>, darkMode: boolean) {
  if (!SHEETS_URL) return false;
  // Activity is deliberately local-only. Uploading an ever-growing event list on
  // every click was the largest source of request size and Apps Script work.
  const payload: Omit<Required<RemoteState>, "activities"> & { version: 2 } = { version: 2, tasks, insights, focusByDate, settings: { darkMode } };
...
function createdDate(task: Task) {
  const fromCreatedAt = task.createdAt ? jakartaDateFrom(task.createdAt) : "";
  return isDateKey(fromCreatedAt) ? fromCreatedAt : isDateKey(task.date) ? task.date : jakartaDate();
}
...
// Older versions used Date.now() as an ID. Tasks created in the same
// millisecond could therefore share an ID, making one delete remove all of
// them. Convert those records once while preserving distinct task objects.
function normalizeTasks(value: unknown): Task[] {
  if (!Array.isArray(value)) return [];
  const seenIds = new Set<string>();
  return value.filter((task): task is Omit<Task, "id"> & { id?: unknown } => !!task && typeof task === "object").map((task) => {
    const candidate = typeof task.id === "string" || typeof task.id === "number" ? String(task.id) : "";
    const id = candidate && !seenIds.has(candidate) ? candidate : createTaskId();
    seenIds.add(id);
    return { ...task, id };
  });
}
...
function streakFromTasks(tasks: Task[]) {
  const createdDates = new Set(tasks.map((task) => task.createdAt ? jakartaDateFrom(task.createdAt) : "").filter(isDateKey));
  let streak = 0;
...
class InsightsErrorBoundary extends Component<{ children: ReactNode }, { failed: boolean }> {
  state = { failed: false };
  static getDerivedStateFromError() { return { failed: true }; }
  componentDidCatch(error: Error, info: ErrorInfo) { console.error("Insights could not be rendered", error, info); }
  render() { return this.state.failed ? <section className="page-panel"><div className="card insight-error"><h1>Insights belum dapat dimuat</h1><p className="muted">Data insight sedang diperbaiki. Kembali ke halaman ini untuk memuat ulang.</p></div></section> : this.props.children; }
}
...
function InsightsView({ tasks, focusSeconds, insights }: { tasks: Task[]; focusSeconds: number; insights: DailyInsight[] }) {
  ...
  const chartData = useMemo(() => {
    const buckets = new Map<string, { total: number; done: number; logins: number; activities: number }>();
    insights.forEach((item) => {
      if (!isDateKey(item.date)) return;
      const key = range === "This year" ? item.date.slice(0, 7) : item.date;
      const bucket = buckets.get(key) || { total: 0, done: 0, logins: 0, activities: 0 };
      bucket.total += Number(item.totalTasks) || 0; bucket.done += Number(item.completedTasks) || 0; bucket.logins += Number(item.loginCount) || 0; bucket.activities += Number(item.activityCount) || 0;
      buckets.set(key, bucket);
    });
    return chartDates.map((key) => { const data = buckets.get(key) || { total: 0, done: 0, logins: 0, activities: 0 }; const value = data.total ? Math.min(100, Math.round((data.done / data.total) * 100)) : 0; return { key, value, count: data.total, logins: data.logins, activities: data.activities, label: range === "This year" ? new Date(`${key}-01T00:00:00`).toLocaleDateString("en-US", { month: "short" }) : shortDateLabel(key) }; });
  }, [chartDates, insights, range]);
...
function App() {
  ...
  const [tasks, setTasks] = useState<Task[]>(() => normalizeTasks(readStored<unknown>(TASK_STORAGE_KEY, []))); const [activities, setActivities] = useState<Activity[]>(() => readStored<Activity[]>(ACTIVITY_STORAGE_KEY, []).slice(-MAX_LOCAL_ACTIVITIES)); const [focusByDate, setFocusByDate] = useState<Record<string, number>>(() => readStored<Record<string, number>>(FOCUS_STORAGE_KEY, {})); const [dailyInsights, setDailyInsights] = useState<DailyInsight[]>(() => hydrateInsights(normalizeTasks(readStored<unknown>(TASK_STORAGE_KEY, [])), readStored<Activity[]>(ACTIVITY_STORAGE_KEY, []), readStored<DailyInsight[]>(INSIGHT_STORAGE_KEY, []), readStored<Record<string, number>>(FOCUS_STORAGE_KEY, {}))); const [filter, setFilter] = useState("Today"); const [taskTitle, setTaskTitle] = useState(""); const [taskPriority, setTaskPriority] = useState<Priority>("Low"); const [taskDate, setTaskDate] = useState(jakartaDate()); const [reminderAt, setReminderAt] = useState(""); const [period, setPeriod] = useState<Period>("Focus"); const [activePeriod, setActivePeriod] = useState<Period | null>(initialTimer.activePeriod); const [timerEndAt, setTimerEndAt] = useState<number | null>(initialTimer.endAt); const [timerValues, setTimerValues] = useState<Record<Period, number>>(initialTimer.values); const [focusStartedAt, setFocusStartedAt] = useState<number | null>(initialTimer.focusStartedAt); const [quoteIndex, setQuoteIndex] = useState(0); const [page, setPage] = useState<"overview" | "calendar" | "insights" | "settings">("overview"); const [darkMode, setDarkMode] = useState(() => readStored("focusflow-settings", { darkMode: false }).darkMode); const [remoteReady, setRemoteReady] = useState(!SHEETS_URL); const [syncStatus, setSyncStatus] = useState<"idle" | "syncing" | "success" | "error">("idle"); const [showTaskForm, setShowTaskForm] = useState(false); const [taskMenu, setTaskMenu] = useState<string | null>(null); const [mobileNav, setMobileNav] = useState(false); const [focusSeconds, setFocusSeconds] = useState(() => readStored<Record<string, number>>(FOCUS_STORAGE_KEY, {})[jakartaDate()] || 0); const [notificationReady, setNotificationReady] = useState(() => "Notification" in window && Notification.permission === "granted"); const [appNotification, setAppNotification] = useState<AppNotification | null>(null); const [deletedTask, setDeletedTask] = useState<DeletedTask | null>(null); const [completingTaskIds, setCompletingTaskIds] = useState<string[]>([]); const focusSecondsRef = useRef(focusSeconds); const tasksRef = useRef(tasks); const activitiesRef = useRef(activities); const insightsRef = useRef(dailyInsights); const focusByDateRef = useRef(focusByDate); const darkModeRef = useRef(darkMode); const syncInFlightRef = useRef(false); const syncQueuedRef = useRef(false);
  const today = jakartaDate(); const todayTasks = useMemo(() => tasks.filter((task) => task.date === today), [tasks, today]); const completed = useMemo(() => todayTasks.reduce((total, task) => total + Number(task.done), 0), [todayTasks]); const visibleTasks = filter === "Completed" ? todayTasks.filter((task) => task.done) : todayTasks.filter((task) => !task.done); const progress = todayTasks.length ? Math.round((completed / todayTasks.length) * 100) : 0; const seconds = timerValues[period]; const time = `${String(Math.floor(seconds / 60)).padStart(2, "0")}:${String(seconds % 60).padStart(2, "0")}`; const timerRemainingPercent = Math.max(0, Math.min(100, (seconds / DURATIONS[period]) * 100)); const timerIsReady = !activePeriod && seconds === DURATIONS[period];
  ...
  useEffect(() => { let cancelled = false; void loadGoogleSheets().then((remote) => { if (cancelled || !remote) return; const remoteTasks = normalizeTasks(remote.tasks); const remoteActivities = Array.isArray(remote.activities) ? remote.activities.slice(-MAX_LOCAL_ACTIVITIES) : activitiesRef.current; const remoteFocus = remote.focusByDate || {}; setTasks(remoteTasks); if (Array.isArray(remote.activities)) setActivities(remoteActivities); setFocusByDate(remoteFocus); setDailyInsights(hydrateInsights(remoteTasks, remoteActivities, Array.isArray(remote.insights) ? remote.insights : [], remoteFocus)); setFocusSeconds(Number(remoteFocus[jakartaDate()]) || 0); if (typeof remote.settings?.darkMode === "boolean") setDarkMode(remote.settings.darkMode); }).catch(() => { /* Local data remains available when offline or the deployment is not readable. */ }).finally(() => { if (!cancelled) setRemoteReady(true); }); return () => { cancelled = true; }; }, []);
  useEffect(() => { if (remoteReady) addActivity("login", "Aplikasi dibuka"); }, [remoteReady]);
  // One in-flight request per browser prevents Apps Script from receiving bursts
  // of overlapping full-state writes. A queued change is sent once afterwards.
  const flushSync = async (manual = false) => {
    if (!SHEETS_URL || !remoteReady) return false;
    if (syncInFlightRef.current) { syncQueuedRef.current = true; return true; }
    syncInFlightRef.current = true;
    if (manual) setSyncStatus("syncing");
    const success = await syncGoogleSheets(tasksRef.current, insightsRef.current, focusByDateRef.current, darkModeRef.current);
    syncInFlightRef.current = false;
    if (manual) setSyncStatus(success ? "success" : "error");
    if (syncQueuedRef.current) { syncQueuedRef.current = false; void flushSync(); }
    return success;
  };
  // Focus time changes every second. It remains local during a running session;
  // non-timer changes are coalesced into one request after five seconds.
  useEffect(() => { if (!SHEETS_URL || !remoteReady) return; const timer = window.setTimeout(() => void flushSync(), SYNC_DEBOUNCE_MS); return () => window.clearTimeout(timer); }, [tasks, dailyInsights, focusByDate, darkMode, remoteReady]);
  useEffect(() => { focusSecondsRef.current = focusSeconds; }, [focusSeconds]);
  // Keep the visible timer smooth without rebuilding the complete insight data
  // every second. Focus totals are checkpointed once a minute and when a session stops.
  useEffect(() => {
    const saveFocusTotal = () => setFocusByDate((current) => current[jakartaDate()] === focusSecondsRef.current ? current : { ...current, [jakartaDate()]: focusSecondsRef.current });
    saveFocusTotal();
    const timer = window.setInterval(saveFocusTotal, 60_000);
    return () => window.clearInterval(timer);
  }, []);
  useEffect(() => { if (!activePeriod) setFocusByDate((current) => current[jakartaDate()] === focusSeconds ? current : { ...current, [jakartaDate()]: focusSeconds }); }, [activePeriod, focusSeconds]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const syncForToday = () => { void flushSync(); localStorage.setItem(SHEETS_LAST_SYNC_KEY, jakartaDate()); };
  ...
  useEffect(() => { if (!deletedTask) return; const timer = window.setTimeout(() => setDeletedTask(null), 8_000); return () => window.clearTimeout(timer); }, [deletedTask]);
  async function handleManualSync() { await flushSync(true); }
  function addTask(event: FormEvent) { event.preventDefault(); if (!taskTitle.trim()) return; const createdAt = new Date().toISOString(); const task: Task = { id: createTaskId(), title: taskTitle.trim(), category: "Personal", priority: taskPriority, done: false, date: taskDate, reminderAt: reminderAt || undefined, createdAt }; if (reminderAt && "Notification" in window && Notification.permission === "default") void Notification.requestPermission().then((permission) => setNotificationReady(permission === "granted")); setTasks((current) => [...current, task]); addActivity("task_created", `Task dibuat: ${task.title}`, task.id); setTaskTitle(""); closeTaskForm(); }
  function toggleTask(id: string) { const task = tasks.find((item) => item.id === id); if (!task || completingTaskIds.includes(id)) return; const applyChange = () => { setTasks((current) => current.map((item) => item.id === id ? { ...item, done: !task.done, completedAt: !task.done ? new Date().toISOString() : undefined } : item)); addActivity(task.done ? "task_reopened" : "task_completed", `${task.done ? "Task dibuka kembali" : "Task selesai"}: ${task.title}`, id); };
    if (!task.done) { setCompletingTaskIds((current) => [...current, id]); window.setTimeout(() => { applyChange(); setCompletingTaskIds((current) => current.filter((taskId) => taskId !== id)); }, 420); return; }
    applyChange();
  }
  function deleteTask(task: Task) { const index = tasks.findIndex((item) => item.id === task.id); if (index < 0) return; setTasks((current) => current.filter((item) => item.id !== task.id)); setTaskMenu(null); setDeletedTask({ task, index }); addActivity("task_deleted", `Task dihapus: ${task.title}`, task.id); }
  function undoDelete() { if (!deletedTask) return; setTasks((current) => current.some((item) => item.id === deletedTask.task.id) ? current : [...current.slice(0, deletedTask.index), deletedTask.task, ...current.slice(deletedTask.index)]); setDeletedTask(null); }
  ...
  return <div className="app-shell">
    ...
    {deletedTask && <aside className="undo-toast" role="status" aria-live="polite"><div><strong>Task dihapus</strong><span>{deletedTask.task.title}</span></div><button onClick={undoDelete}>Undo</button><button className="dismiss-undo" onClick={() => setDeletedTask(null)} aria-label="Tutup notifikasi">×</button></aside>}
    ...
    <main className="content">{page === "calendar" ? <CalendarView tasks={tasks} openNewTask={openNewTask}/> : page === "insights" ? <InsightsErrorBoundary><InsightsView tasks={tasks} focusSeconds={focusSeconds} insights={dailyInsights}/></InsightsErrorBoundary> : page === "settings" ? <SettingsView darkMode={darkMode} setDarkMode={setDarkMode} addActivity={addActivity}/> : <>
      ...
      <div className="workspace-grid"><section className="tasks-card card"><div className="section-heading"><div><h2>Today&apos;s tasks</h2><p>{todayTasks.filter((task) => !task.done).length} tasks remaining</p></div></div><div className="filters">{["Today", "Completed"].map((item) => <button onClick={() => setFilter(item)} className={filter === item ? "selected" : ""} key={item}>{item}{item === "Completed" && <b className="completed-count" aria-label={`${completed} task selesai`}>{completed}</b>}</button>)}</div><div className="task-list">{visibleTasks.length ? visibleTasks.map((task) => { const isCompleting = completingTaskIds.includes(task.id); return <div className={`task-row ${task.done ? "complete" : ""} ${isCompleting ? "is-completing" : ""}`} key={task.id} aria-busy={isCompleting}><button className="checkbox" onClick={() => toggleTask(task.id)} aria-label={`Toggle ${task.title}`} disabled={isCompleting}>{task.done && "✓"}</button><div className="task-copy"><strong>{task.title}</strong><span>Created {dateLabel(createdDate(task))} <b className={task.date < today ? "due-badge overdue" : task.date === today ? "due-badge today" : "due-badge"}>Due {taskDateDescription(task.date)}</b></span></div><span className={`priority ${task.priority.toLowerCase()}`}>{task.priority}</span><div className="task-actions"><button className="more" onClick={() => setTaskMenu(taskMenu === task.id ? null : task.id)} aria-label={`Aksi untuk ${task.title}`} disabled={isCompleting}><Icon name="more"/></button>{taskMenu === task.id && <div className="task-menu"><button onClick={() => toggleTask(task.id)}>{task.done ? "Mark active" : "Mark complete"}</button><button onClick={() => deleteTask(task)}>Delete task</button></div>}</div></div>; }) : <div className="empty-task-state"><span>✦</span><strong>{filter === "Completed" ? "Belum ada task yang selesai" : "Hari ini masih terasa lapang"}</strong><p>{filter === "Completed" ? "Selesaikan satu task untuk melihat progresmu di sini." : "Mulai dengan satu hal kecil yang paling penting."}</p>{filter === "Today" && <button onClick={() => openNewTask(today)}><Icon name="plus" size={16}/>Tambah task pertama</button>}</div>}</div></section>
      <aside className="right-column"><section className="timer-card card"><div className="timer-heading"><div><h2>Focus timer</h2><p>{activePeriod ? `${activePeriod} is running` : "Stay in the zone"}</p></div></div><div className="timer-tabs">{(Object.keys(DURATIONS) as Period[]).map((item) => <button onClick={() => setPeriod(item)} className={period === item ? "active" : ""} key={item}>{item}</button>)}</div><div className="timer-wrap"><div className={timerIsReady ? "timer-ring is-ready" : "timer-ring"}><svg viewBox="0 0 168 168" aria-hidden="true"><circle className="ring-track" cx="84" cy="84" r="78"/><circle className="ring-progress" cx="84" cy="84" r="78" pathLength="100" style={{ strokeDasharray: timerIsReady ? "100 0" : `${timerRemainingPercent} 100` }}/></svg>{!timerIsReady && <span className="ring-marker" style={{ transform: `rotate(${timerRemainingPercent * 3.6}deg)` }}/>}<div><strong>{time}</strong><span>{period}</span></div></div></div><div className="timer-controls"><button className="reset" onClick={() => { if (activePeriod === period) { if (period === "Focus") { commitFocusSession(focusStartedAt); setFocusStartedAt(null); } setActivePeriod(null); setTimerEndAt(null); } setTimerValues((values) => ({ ...values, [period]: DURATIONS[period] })); addActivity("timer_reset", `Timer ${period} direset`); }}><Icon name="rotate"/></button><button className="start" onClick={() => { const stopping = activePeriod === period; if (stopping) { if (period === "Focus") { commitFocusSession(focusStartedAt); setFocusStartedAt(null); } setActivePeriod(null); setTimerEndAt(null); } else { const startedAt = Date.now(); if (activePeriod === "Focus") commitFocusSession(focusStartedAt); setActivePeriod(period); setTimerEndAt(startedAt + timerValues[period] * 1000); setFocusStartedAt(period === "Focus" ? startedAt : null); if ("Notification" in window && Notification.permission === "default") void Notification.requestPermission().then((permission) => setNotificationReady(permission === "granted")); } addActivity(stopping ? "focus_paused" : "focus_started", `${period} ${stopping ? "dijeda" : "dimulai"}`); }}><Icon name={activePeriod === period ? "pause" : "play"}/>{activePeriod === period ? "Pause" : "Start"}</button></div></section><section className="quote-card"><span>“</span><div className="quote-content"><p>{QUOTES[quoteIndex].text}</p><small>— {QUOTES[quoteIndex].author}</small></div><div className="quote-controls"><button onClick={() => setQuoteIndex((quoteIndex - 1 + QUOTES.length) % QUOTES.length)}>←</button><button onClick={() => setQuoteIndex((quoteIndex + 1) % QUOTES.length)}>→</button></div></section></aside></div>
    </>}</main>
