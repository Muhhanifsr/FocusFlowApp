...
function App() {
  ...
  useEffect(() => { let cancelled = false; void loadGoogleSheets().then((remote) => { if (cancelled || !remote) return; const remoteTasks = Array.isArray(remote.tasks) ? remote.tasks : []; const remoteActivities = Array.isArray(remote.activities) ? remote.activities.slice(-MAX_LOCAL_ACTIVITIES) : []; const remoteFocus = remote.focusByDate || {}; setTasks(remoteTasks); setActivities(remoteActivities); setFocusByDate(remoteFocus); setDailyInsights(hydrateInsights(remoteTasks, remoteActivities, Array.isArray(remote.insights) ? remote.insights : [], remoteFocus)); setFocusSeconds(Number(remoteFocus[jakartaDate()]) || 0); if (typeof remote.settings?.darkMode === "boolean") setDarkMode(remote.settings.darkMode); }).catch(() => { /* Local data remains available when offline or the deployment is not readable. */ }).finally(() => { if (!cancelled) setRemoteReady(true); }); return () => { cancelled = true; }; }, []);
  useEffect(() => { if (remoteReady) addActivity("login", "Aplikasi dibuka"); }, [remoteReady]);
  // Focus time changes every second. It remains local during a running session and
  // is sent at the daily checkpoint/manual sync instead of uploading a full snapshot each second.
  useEffect(() => { if (!SHEETS_URL || !remoteReady) return; const timer = window.setTimeout(() => syncGoogleSheets(tasksRef.current, insightsRef.current, activitiesRef.current, focusByDateRef.current, darkModeRef.current), 1_500); return () => window.clearTimeout(timer); }, [tasks, activities, dailyInsights, darkMode, remoteReady]);
